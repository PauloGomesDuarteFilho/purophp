...
<div id="aparelhos">
	...
	<div class="aparelhos-content">
		<?php foreach ($blog_posts as $posts) { ?>
			<article>
				...
				<h3 class="Cycle fs-16 fw400"><?=BDtoBR($posts['datacad'])?> por <b>Dental Arte</b><?php if($posts['categoria'] != null) { ?> | <b><?=Categorias($posts['categoria'])?></b><?php } ?></h3>
				<?php if($posts['imagem'] != null) { ?><a name="imagem_destacada"><img src="admin/images/<?=$posts['imagem']?>" class="autow" alt="<?=$posts['titulo']?>"></a><?php } ?>			
				<div class="p-topo Cycle fs-20" style="margin:25px 0 50px 0"><?=$posts['conteudo']?></div>
			</article>
		<?php } ?>
	</div>
	...
	<div class="cleary">
		<div id="aparelhos-titulo-pagina">
			<p class="Oswald fs-38 azul fw700">continuar lendo</p>
		</div>
		<div id="rp">
		<?php foreach ($related as $relacionados) { ?>
			<div class="w_280">
				<?php if($relacionados['imagem'] != null) { ?>
					<a href="blog/<?=$relacionados['url_amigavel']?>" name="imagem_destacada">
						<img src="admin/images/<?=$relacionados['imagem']?>" class="autow" alt="<?=$relacionados['titulo']?>">
					</a>
				<?php } ?>
				<h3 class="Cycle fs-20 cinza bolder">
					<a href="blog/<?=$relacionados['url_amigavel']?>" style="color:#3f3f3f !important">
						<?=$relacionados['titulo']?>
					</a>
				</h3>
				<div class="Cycle fs-16 cinza">
					<?=limit_text($relacionados['conteudo'], 30); ?>
				</div>
				<a href="blog/<?=$relacionados['url_amigavel']?>">
